...
process_cancer_type <- function(cancer_type, sample_info_subset) {
  ...
  if (length(annotated_objects) > 0) {
    ...
    # 生成质控图表
    tryCatch({
      generate_qc_plots(combined_seurat, cancer_output_dir, cancer_type)
      message("✓ 质控图表生成完成")
    }, error = function(e) {
      warning("质控图表生成失败: ", e$message)
    })
...
# 生成质控图表
# 改进的质控图表生成函数
generate_qc_plots <- function(seurat_obj, output_dir, cancer_type) {
  message("生成质控图表...")
  ...
  if (!dir.exists(plot_dir)) {
    warning("无法创建plots目录: ", plot_dir)
    return(FALSE)
  }
  message("✓ plots目录创建成功: ", plot_dir)
  
  # 检查Seurat对象基本信息
  message("Seurat对象信息: ", ncol(seurat_obj), " 个细胞, ", nrow(seurat_obj), " 个基因")
  
  # 检查元数据
  required_columns <- c("fine_cell_type", "sample_id")
  missing_columns <- setdiff(required_columns, colnames(seurat_obj@meta.data))
  if (length(missing_columns) > 0) {
    warning("缺少必要的元数据列: ", paste(missing_columns, collapse = ", "))
    return(FALSE)
  }
  ...
  message("✓ UMAP坐标验证成功: ", nrow(umap_coords), " 个细胞")
  
  # 清理细胞类型名称中的特殊字符
  seurat_obj$fine_cell_type_clean <- gsub("[^A-Za-z0-9_.-]", "_", seurat_obj$fine_cell_type)
  seurat_obj$sample_id_clean <- gsub("[^A-Za-z0-9_.-]", "_", seurat_obj$sample_id)
  
  # 统计信息
  n_cell_types <- length(unique(seurat_obj$fine_cell_type))
  n_samples <- length(unique(seurat_obj$sample_id))
  message("细胞类型数: ", n_cell_types, ", 样本数: ", n_samples)
  ...
  tryCatch({
    ...
    p1 <- DimPlot(seurat_obj, 
                  group.by = "fine_cell_type_clean", 
                  label = TRUE, 
                  repel = TRUE,
                  pt.size = 0.5,
                  label.size = 3) +
      ggtitle(paste0(cancer_type, " - Fine Cell Types (", n_cell_types, " types)")) +
      theme_classic() +
      theme(
    ...
    # 保存PDF
    pdf_file1 <- file.path(plot_dir, paste0(cancer_type, "_fine_annotation_umap.pdf"))
    ggsave(pdf_file1, plot = p1, width = 15, height = 12, device = "pdf")
    
    # 验证文件保存
    if (file.exists(pdf_file1)) {
      file_size1 <- file.size(pdf_file1)
      message("✓ 细胞类型UMAP图已保存: ", pdf_file1, " (", round(file_size1/1024, 2), " KB)")
    } else {
      warning("⚠️ 细胞类型UMAP图保存失败")
    }
    
    # 也保存PNG格式
    png_file1 <- file.path(plot_dir, paste0(cancer_type, "_fine_annotation_umap.png"))
    ggsave(png_file1, plot = p1, width = 15, height = 12, device = "png", dpi = 300)
    
    options(warn = 0)  # 恢复警告
    
  }, error = function(e) {
    warning("细胞类型UMAP图生成失败: ", e$message)
    print(traceback())
  })
  ...
  tryCatch({
    ...
    p2 <- DimPlot(seurat_obj, 
                  group.by = "sample_id_clean", 
                  label = TRUE,
                  pt.size = 0.5,
                  label.size = 3) +
      ggtitle(paste0(cancer_type, " - Samples (", n_samples, " samples)")) +
      theme_classic() +
      theme(
        plot.title = element_text(hjust = 0.5, size = 14),
        axis.text = element_text(size = 10),
    ...
    # 保存PDF
    pdf_file2 <- file.path(plot_dir, paste0(cancer_type, "_samples_umap.pdf"))
    ggsave(pdf_file2, plot = p2, width = 14, height = 10, device = "pdf")
    
    # 验证文件保存
    if (file.exists(pdf_file2)) {
      file_size2 <- file.size(pdf_file2)
      message("✓ 样本UMAP图已保存: ", pdf_file2, " (", round(file_size2/1024, 2), " KB)")
    } else {
      warning("⚠️ 样本UMAP图保存失败")
    }
    
    # 也保存PNG格式
    png_file2 <- file.path(plot_dir, paste0(cancer_type, "_samples_umap.png"))
    ggsave(png_file2, plot = p2, width = 14, height = 10, device = "png", dpi = 300)
    
    options(warn = 0)
    
  }, error = function(e) {
    warning("样本UMAP图生成失败: ", e$message)
    print(traceback())
  })
  ...
  tryCatch({
    # 创建比例数据
    cell_props <- table(seurat_obj$fine_cell_type_clean, seurat_obj$sample_id_clean)
    
    # 检查数据
    if (nrow(cell_props) == 0 || ncol(cell_props) == 0) {
      warning("比例数据为空")
      return(FALSE)
    }
    
    cell_props_pct <- prop.table(cell_props, margin = 2) * 100
    
    # 转换为长格式数据
    props_df <- as.data.frame(cell_props_pct)
    colnames(props_df) <- c("CellType", "Sample", "Percentage")
    
    # 移除百分比为0的行以简化图表
    props_df <- props_df[props_df$Percentage > 0, ]
    
    message("比例数据维度: ", nrow(props_df), " 行")
    
    if (nrow(props_df) > 0) {
      # 限制显示的细胞类型数量（取前20个最常见的）
      top_celltypes <- names(sort(table(seurat_obj$fine_cell_type_clean), decreasing = TRUE))[1:min(20, n_cell_types)]
      props_df_filtered <- props_df[props_df$CellType %in% top_celltypes, ]
      
      p3 <- ggplot(props_df_filtered, aes(x = Sample, y = Percentage, fill = CellType)) +
        ...
        theme(
          ...
          axis.title = element_text(size = 12),
          plot.title = element_text(hjust = 0.5, size = 14),
          legend.position = "none"  # 隐藏图例以避免过于拥挤
        ) +
        labs(
          x = "Sample",
          y = "Percentage (%)",
          title = paste0(cancer_type, " - Cell Type Proportions")
        ) +
        scale_y_continuous(expand = c(0, 0))
      
      # 保存PDF
      pdf_file3 <- file.path(plot_dir, paste0(cancer_type, "_cell_proportions.pdf"))
      ggsave(pdf_file3, plot = p3, width = max(12, n_samples * 0.8), height = 10, device = "pdf")
      
      # 验证文件保存
      if (file.exists(pdf_file3)) {
        file_size3 <- file.size(pdf_file3)
        message("✓ 细胞类型比例图已保存: ", pdf_file3, " (", round(file_size3/1024, 2), " KB)")
      } else {
        warning("⚠️ 细胞类型比例图保存失败")
      }
      
      # 也保存PNG格式
      png_file3 <- file.path(plot_dir, paste0(cancer_type, "_cell_proportions.png"))
      ggsave(png_file3, plot = p3, width = max(12, n_samples * 0.8), height = 10, device = "png", dpi = 300)
  ...
  tryCatch({
    ...
    summary_stats <- data.frame(
      CellType = names(table(seurat_obj$fine_cell_type)),
      Count = as.numeric(table(seurat_obj$fine_cell_type)),
      Percentage = round(as.numeric(table(seurat_obj$fine_cell_type)) / ncol(seurat_obj) * 100, 2)
    )
    summary_stats <- summary_stats[order(summary_stats$Count, decreasing = TRUE), ]
    
    summary_file <- file.path(output_dir, paste0(cancer_type, "_summary_stats.csv"))
    write.csv(summary_stats, summary_file, row.names = FALSE)
    
    if (file.exists(summary_file)) {
      message("✓ 统计汇总已保存: ", summary_file)
    } else {
      warning("⚠️ 统计汇总保存失败")
    }
    ...
    sample_stats <- data.frame(
      Sample = names(table(seurat_obj$sample_id)),
      N_Cells = as.numeric(table(seurat_obj$sample_id)),
      N_CellTypes = sapply(names(table(seurat_obj$sample_id)), function(s) {
        length(unique(seurat_obj$fine_cell_type[seurat_obj$sample_id == s]))
      })
    )
    
    sample_file <- file.path(output_dir, paste0(cancer_type, "_sample_stats.csv"))
    write.csv(sample_stats, sample_file, row.names = FALSE)
    
    if (file.exists(sample_file)) {
      message("✓ 样本统计已保存: ", sample_file)
    }
    
  }, error = function(e) {
    warning("统计汇总保存失败: ", e$message)
  })
