...
# 精细化注释函数
perform_fine_annotation <- function(seurat_obj, major_cell_type, cell_subset) {
...
process_cancer_type <- function(cancer_type, sample_info_subset) {
  ...
  # 修正 process_cancer_type 函数中的关键部分
for (major_type in major_types) {
    ...
    # 进行精细化注释 - 修正：传入 cell_subset 而不是 combined_seurat
    cell_subset <- perform_fine_annotation(major_cell_type, cell_subset)  # 修正这里
  ...
  if (length(annotated_objects) > 0) {
    ...
    # 保存结果
    message("保存结果...")
    saveRDS(combined_seurat, file.path(cancer_output_dir, paste0(cancer_type, "_fine_annotated.rds")))
    
    # 保存细胞类型统计
    cell_type_stats <- table(combined_seurat$fine_cell_type, combined_seurat$sample_id)
    write.csv(cell_type_stats, file.path(cancer_output_dir, paste0(cancer_type, "_cell_type_stats.csv")))
    
    # 保存每个主要细胞类型的详细结果
    for (major_type in names(annotated_objects)) {
      saveRDS(annotated_objects[[major_type]], 
              file.path(cancer_output_dir, paste0(cancer_type, "_", major_type, "_detailed.rds")))
    }
...
generate_qc_plots <- function(seurat_obj, output_dir, cancer_type) {
  ...
  # 细胞类型分布图
  p1 <- DimPlot(seurat_obj, group.by = "fine_cell_type", label = TRUE, repel = TRUE) +
    ggtitle(paste0(cancer_type, " - Fine Cell Types")) +
    theme(legend.position = "none")
  
  p2 <- DimPlot(seurat_obj, group.by = "sample_id", label = TRUE) +
    ggtitle(paste0(cancer_type, " - Samples"))
  
  # 保存图片
  ggsave(file.path(plot_dir, paste0(cancer_type, "_fine_annotation_umap.pdf")), 
         p1, width = 15, height = 12)
  ggsave(file.path(plot_dir, paste0(cancer_type, "_samples_umap.pdf")), 
         p2, width = 12, height = 10)
  
  # 细胞类型比例图
  cell_props <- table(seurat_obj$fine_cell_type, seurat_obj$sample_id)
  ...
  p3 <- ggplot(props_df, aes(x = Sample, y = Percentage, fill = CellType)) +
    geom_bar(stat = "identity") +
    theme_classic() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          legend.position = "none") +
    ggtitle(paste0(cancer_type, " - Cell Type Proportions"))
  
  ggsave(file.path(plot_dir, paste0(cancer_type, "_cell_proportions.pdf")), 
  ...
  write.csv(summary_stats, file.path(output_dir, paste0(cancer_type, "_summary_stats.csv")), 
            row.names = FALSE)
}
