...
generate_qc_plots <- function(seurat_obj, output_dir, cancer_type) {
  ...
  message("  可用assay: ", paste(names(seurat_obj@assays), collapse = ", "))
  message("  可用降维: ", paste(names(seurat_obj@reductions), collapse = ", "))
  message("  元数据列: ", paste(colnames(seurat_obj@meta.data), collapse = ", "))
  ...
  for (col in required_columns) {
    if (col %in% colnames(seurat_obj@meta.data)) {
      ...
      message("  ✓ ", col, ": ", n_unique, " 个唯一值")
      
      # 显示前几个值
      unique_vals <- unique(seurat_obj@meta.data[[col]])
      message("    前5个值: ", paste(head(unique_vals, 5), collapse = ", "))
    } else {
      warning("  ✗ 缺少列: ", col)
      return(FALSE)
    }
  ...
  if ("umap" %in% names(seurat_obj@reductions)) {
    ...
    message("  UMAP坐标维度: ", dim(umap_coords)[1], " x ", dim(umap_coords)[2])
    message("  UMAP坐标范围: X[", round(min(umap_coords[,1]), 2), ", ", round(max(umap_coords[,1]), 2), "], Y[", round(min(umap_coords[,2]), 2), ", ", round(max(umap_coords[,2]), 2), "]")
  } else {
    warning("UMAP降维失败，无法生成图表")
    return(FALSE)
  }
  
  # 测试基本绘图功能
  message("测试基本绘图功能...")
  test_plot_file <- file.path(plot_dir, "test_plot.pdf")
  tryCatch({
    pdf(test_plot_file, width = 5, height = 5)
    plot(1:10, 1:10, main = "Test Plot")
    dev.off()
    
    if (file.exists(test_plot_file)) {
      file_size <- file.size(test_plot_file)
      message("✓ 基本绘图功能正常: ", test_plot_file, " (", file_size, " bytes)")
      # 清理测试文件
    ...
    warning("✗ 基本绘图功能失败: ", e$message)
    return(FALSE)
  })
  
  # 测试ggplot2基本功能
  message("测试ggplot2功能...")
  test_ggplot_file <- file.path(plot_dir, "test_ggplot.pdf")
  tryCatch({
    library(ggplot2)
    p_test <- ggplot(data.frame(x = 1:10, y = 1:10), aes(x = x, y = y)) + 
      geom_point() + 
      ggtitle("Test ggplot")
    
    ggsave(test_ggplot_file, plot = p_test, width = 5, height = 5, device = "pdf")
    
    if (file.exists(test_ggplot_file)) {
      file_size <- file.size(test_ggplot_file)
      message("✓ ggplot2功能正常: ", test_ggplot_file, " (", file_size, " bytes)")
      # 清理测试文件
      unlink(test_ggplot_file)
    } else {
      warning("✗ ggplot2测试文件未创建")
      return(FALSE)
    }
  }, error = function(e) {
    warning("✗ ggplot2功能失败: ", e$message)
    print(e)
    return(FALSE)
  })
  
  # 现在尝试生成实际的Seurat图表
  message("开始生成Seurat图表...")
  
  # 1. 最简单的DimPlot测试
  message("1. 生成简单的DimPlot...")
  simple_plot_file <- file.path(plot_dir, paste0(cancer_type, "_simple_umap.pdf"))
  tryCatch({
    p_simple <- DimPlot(seurat_obj, reduction = "umap") + ggtitle("Simple UMAP")
    ggsave(simple_plot_file, plot = p_simple, width = 8, height = 6, device = "pdf")
    
    if (file.exists(simple_plot_file)) {
      file_size <- file.size(simple_plot_file)
      message("✓ 简单UMAP图生成成功: ", simple_plot_file, " (", file_size, " bytes)")
    } else {
      warning("✗ 简单UMAP图生成失败")
    }
  }, error = function(e) {
    warning("简单UMAP图生成失败: ", e$message)
    print(e)
  })
  
  # 2. 按细胞类型着色的DimPlot
  message("2. 生成细胞类型UMAP图...")
  celltype_plot_file <- file.path(plot_dir, paste0(cancer_type, "_celltype_umap.pdf"))
  tryCatch({
    # 清理细胞类型名称
    seurat_obj$fine_cell_type_clean <- gsub("[^A-Za-z0-9_.-]", "_", seurat_obj$fine_cell_type)
    
    n_types <- length(unique(seurat_obj$fine_cell_type_clean))
    message("  细胞类型数量: ", n_types)
    
    # 检查是否有太多细胞类型
    if (n_types > 50) {
      message("  细胞类型过多，将只显示前20个最常见的类型")
      top_types <- names(sort(table(seurat_obj$fine_cell_type_clean), decreasing = TRUE))[1:20]
      seurat_obj$fine_cell_type_display <- ifelse(seurat_obj$fine_cell_type_clean %in% top_types, 
                                                  seurat_obj$fine_cell_type_clean, "Other")
      group_var <- "fine_cell_type_display"
    } else {
      group_var <- "fine_cell_type_clean"
    }
    
    p_celltype <- DimPlot(seurat_obj, 
                         group.by = group_var,
                         reduction = "umap",
                         pt.size = 0.3) + 
      ggtitle(paste0(cancer_type, " - Cell Types")) +
      theme(legend.position = "right")
    
    ggsave(celltype_plot_file, plot = p_celltype, width = 12, height = 8, device = "pdf")
    
    if (file.exists(celltype_plot_file)) {
      file_size <- file.size(celltype_plot_file)
      message("✓ 细胞类型UMAP图生成成功: ", celltype_plot_file, " (", file_size, " bytes)")
    } else {
      warning("✗ 细胞类型UMAP图生成失败")
    }
  }, error = function(e) {
  ...
  # 3. 按样本着色的DimPlot
  message("3. 生成样本UMAP图...")
  sample_plot_file <- file.path(plot_dir, paste0(cancer_type, "_sample_umap.pdf"))
  tryCatch({
    seurat_obj$sample_id_clean <- gsub("[^A-Za-z0-9_.-]", "_", seurat_obj$sample_id)
    
    n_samples <- length(unique(seurat_obj$sample_id_clean))
    message("  样本数量: ", n_samples)
    
    p_sample <- DimPlot(seurat_obj, 
                       group.by = "sample_id_clean",
                       reduction = "umap",
                       pt.size = 0.3) + 
      ggtitle(paste0(cancer_type, " - Samples")) +
      theme(legend.position = "right")
    
    ggsave(sample_plot_file, plot = p_sample, width = 12, height = 8, device = "pdf")
    
    if (file.exists(sample_plot_file)) {
      file_size <- file.size(sample_plot_file)
      message("✓ 样本UMAP图生成成功: ", sample_plot_file, " (", file_size, " bytes)")
    } else {
      warning("✗ 样本UMAP图生成失败")
    }
  }, error = function(e) {
  ...
  tryCatch({
    ...
    summary_stats <- data.frame(
      ...
      Percentage = round(as.numeric(cell_type_counts) / ncol(seurat_obj) * 100, 2)
    )
    ...
    if (file.exists(summary_file)) {
      message("✓ 统计文件生成成功: ", summary_file)
    } else {
      warning("✗ 统计文件生成失败")
    }
    
  }, error = function(e) {
    warning("统计文件生成失败: ", e$message)
  })
  ...
  if (length(plot_files) > 0) {
    ...
    for (f in plot_files) {
      ...
      message("  ", basename(f), " (", round(file_size/1024, 2), " KB)")
    }
  } else {
    warning("✗ 没有生成任何PDF文件")
  }
