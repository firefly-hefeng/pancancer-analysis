# trajectory_analysis_v19_fixed.R
# 修复变量作用域问题的版本
...
trajectory_output_dir <- file.path(output_base_dir, "trajectory_analysis_2")
...
generate_qc_report <- function(cancer_type, cell_type, seurat_obj, diversity_results, cds, tradeseq_results, trajectory_genes) {
  ...
  safe_execute({
    ...
    cat("=== 包可用性 ===\n")
    cat("tradeSeq:", ifelse(tradeseq_available, "可用", "不可用"), "\n")
    cat("CytoTRACE:", ifelse(cytotrace_available, "可用", "不可用"), "\n\n")
    ...
    cat("tradeSeq:", ifelse(!is.null(tradeseq_results), "成功", "跳过/失败"), "\n")
...
message("\n轨迹分析脚本v19修复版加载完成!")
message("修复问题:")
message("1. 将trajectory_cell_types定义移至脚本顶部作为全局变量")
message("2. 修复变量作用域问题")
message("3. 增强的可视化功能保持不变")