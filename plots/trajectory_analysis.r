...
calculate_gene_diversity_score <- function(seurat_obj, cancer_type, cell_type) {
  ...
  return(safe_execute({
    ...
    max_cells_for_diversity <- 5000  # 进一步减少
    ...
    if (n_genes > 800) {
      if (length(VariableFeatures(subset_obj)) > 0) {
        var_genes <- VariableFeatures(subset_obj)[1:min(800, length(VariableFeatures(subset_obj)))]
        expr_matrix <- expr_matrix[var_genes, ]
      } else {
        gene_means <- Matrix::rowMeans(expr_matrix)
        top_genes <- names(sort(gene_means, decreasing = TRUE))[1:800]
...
run_monocle3_analysis <- function(seurat_obj, cancer_type, cell_type) {
  ...
  return(safe_execute({
    # 数据采样
    n_cells <- ncol(seurat_obj)
    max_cells_for_monocle <- 1000  # 大幅减少
    ...
    # 使用k-means创建非常少的初始聚类
    set.seed(42)
    initial_k <- min(8, floor(ncol(seurat_obj) / 100))  # 每100个细胞一个聚类
    initial_k <- max(initial_k, 4)  # 至少4个聚类
    ...
    # 进一步简化基因集
    if (nrow(expression_matrix) > 2000) {
      if (length(VariableFeatures(seurat_obj)) > 0) {
        var_genes <- VariableFeatures(seurat_obj)[1:min(2000, length(VariableFeatures(seurat_obj)))]
        expression_matrix <- expression_matrix[var_genes, ]
      } else {
        gene_means <- Matrix::rowMeans(expression_matrix)
        top_genes <- names(sort(gene_means, decreasing = TRUE))[1:2000]
    ...
    message("  预处理CDS对象...")
    cds <- preprocess_cds(cds, num_dim = 8, verbose = FALSE, method = "PCA")
...
run_tradeseq_analysis <- function(cds, cancer_type, cell_type) {
  ...
  return(safe_execute({
    ...
    if (nrow(counts_filtered) < 30) {
      stop("可分析基因数过少")
    }
    
    if (nrow(counts_filtered) > 100) {
      set.seed(42)
      sample_genes <- sample(rownames(counts_filtered), 100)
...
analyze_trajectory_genes_simple <- function(cds, cancer_type, cell_type) {
  ...
  return(safe_execute({
    ...
    # 限制基因数量
    if (nrow(expr_matrix) > 80) {
      gene_means <- Matrix::rowMeans(expr_matrix)
      top_genes <- names(sort(gene_means, decreasing = TRUE))[1:80]
      expr_matrix <- expr_matrix[top_genes, ]
    }
...
# 获取细胞类型到聚类的映射
get_cluster_cell_type_mapping <- function(seurat_obj, cds) {
  if (is.null(cds)) {
  ...
  cluster_mapping$cluster_label <- paste0("C", cluster_mapping$cluster, "\n", 
                                         gsub(".*_", "", cluster_mapping$dominant_cell_type),
                                         "\n(n=", cluster_mapping$cell_count, ")")
...
# 简化的可视化函数
generate_trajectory_plots <- function(seurat_obj, cds, diversity_results, cancer_type, cell_type) {
  message("生成轨迹可视化图...")
  ...
  return(safe_execute({
    ...
    if (!is.null(cds)) {
      if ("pseudotime" %in% colnames(colData(cds))) {
        ...
        if (sum(valid_pt) > 0) {
          ...
          p2 <- ggplot(plot_data, aes(x = UMAP_1, y = UMAP_2, color = Pseudotime)) +
            ...
            ggtitle(paste(cancer_type, "-", gsub("_", " ", cell_type)),
                    subtitle = "Simplified Cell State Transition") +
            base_theme
          
          ggsave(paste0(plot_base, "_pseudotime.png"), p2, width = 12, height = 10, dpi = 300, bg = "white")
          ggsave(paste0(plot_base, "_pseudotime.pdf"), p2, width = 12, height = 10, bg = "white")
        }
      }
      ...
      if ("manual_clusters" %in% colnames(colData(cds))) {
        ...
        p3 <- ggplot(plot_data, aes(x = UMAP_1, y = UMAP_2, color = Cluster)) +
          geom_point(size = 0.8, alpha = 0.8) +
          scale_color_brewer(name = "Cluster", palette = "Set3") +
          ggtitle(paste(cancer_type, "-", gsub("_", " ", cell_type)),
                  subtitle = "Cluster Annotation") +
          base_theme
        
        ggsave(paste0(plot_base, "_clusters.png"), p3, width = 14, height = 10, dpi = 300, bg = "white")
        ggsave(paste0(plot_base, "_clusters.pdf"), p3, width = 14, height = 10, bg = "white")
        
        cluster_info <- get_cluster_cell_type_mapping(seurat_obj, cds)
    ...
    if ("target_cell_type" %in% colnames(seurat_obj@meta.data)) {
      p4 <- DimPlot(seurat_obj, group.by = "target_cell_type", reduction = "umap") +
        ...
        ggtitle(paste(cancer_type, "-", gsub("_", " ", cell_type)),
                subtitle = "Cell Subtype Distribution") +
        base_theme
      
      ggsave(paste0(plot_base, "_celltypes.png"), p4, width = 12, height = 8, dpi = 300, bg = "white")
      ggsave(paste0(plot_base, "_celltypes.pdf"), p4, width = 12, height = 8, bg = "white")
    }
    
    message(paste("  轨迹图已保存:", paste0(plot_base, "*")))
    
  }, "visualization", cancer_type, cell_type, NULL))
}

# 生成质控报告
generate_qc_report <- function(cancer_type, cell_type, seurat_obj, diversity_results, cds, tradeseq_results, trajectory_genes) {
  qc_file <- file.path(trajectory_output_dir, "qc_reports", 
                      paste0(cancer_type, "_", cell_type, "_qc_report.txt"))
  ...
  safe_execute({
    ...
    cat("\n=== 分析成功情况 ===\n")
    cat("基因多样性分析:", ifelse(!is.null(diversity_results), "成功", "失败"), "\n")
    cat("Monocle3简化版:", ifelse(!is.null(cds), "成功", "失败"), "\n")
    cat("tradeSeq:", ifelse(!is.null(tradeseq_results), "成功", "失败"), "\n")
    cat("基因分析:", ifelse(!is.null(trajectory_genes) && nrow(trajectory_genes) > 0, "成功", "失败"), "\n")
    
    if (!is.null(cds)) {
      cat("分析细胞数:", ncol(cds), "\n")
      
      if ("manual_clusters" %in% colnames(colData(cds))) {
        cat("最终聚类数:", length(unique(colData(cds)$manual_clusters)), "\n")
      } else {
        cat("最终聚类数: 无法获取\n")
      }
      ...
      if ("pseudotime" %in% colnames(colData(cds))) {
        pseudotime_values <- colData(cds)$pseudotime
        valid_pt <- sum(!is.na(pseudotime_values) & is.finite(pseudotime_values))
        cat("Pseudotime有效率:", round(valid_pt / length(pseudotime_values) * 100, 1), "%\n")
        
        if (valid_pt > 0) {
          cat("Pseudotime范围:", paste(round(range(pseudotime_values, na.rm = TRUE), 2), collapse = " - "), "\n")
        }
      } else {
        cat("Pseudotime: 未成功计算\n")
      }
    }
    
    if (!is.null(diversity_results) && "normalized_diversity" %in% colnames(seurat_obj@meta.data)) {
      cat("\n多样性评分范围:", 
          paste(round(range(seurat_obj$normalized_diversity, na.rm = TRUE), 3), collapse = " - "), "\n")
    }
    
    if (!is.null(tradeseq_results)) {
      cat("tradeSeq显著基因数:", length(tradeseq_results$sig_genes), "\n")
    }
    
    if (!is.null(trajectory_genes) && nrow(trajectory_genes) > 0) {
      sig_count <- sum(trajectory_genes$q_value < 0.05, na.rm = TRUE)
      cat("简化基因分析显著基因数:", sig_count, "\n")
    }
    
    cat("\n=== 报告结束 ===\n")
    sink()
    
  }, "qc_report", cancer_type, cell_type, NULL)
}

# 主分析函数（单个细胞类型）
analyze_single_cell_type <- function(cancer_type, cell_type, force_rerun = FALSE) {
  ...
  # 6. 可视化
  message("步骤6: 生成可视化")
  generate_trajectory_plots(seurat_obj, cds, diversity_data, cancer_type, cell_type)
...
# 定义需要分析的细胞类型
trajectory_cell_types <- c("T_cells", "Myeloid", "B_cell", "NK_cell", 
                          "Epithelial_cells", "Endothelial_cells", "Fibroblasts")

# 主分析函数
main_trajectory_analysis <- function(force_rerun = FALSE) {
  message("开始细胞状态转换轨迹分析...")
  ...
  for (cancer_type in cancer_types) {
    ...
    for (cell_type in cell_types_to_analyze) {
      ...
      result <- analyze_single_cell_type(cancer_type, cell_type, force_rerun)
...
if (!interactive()) {
  message("开始执行轨迹分析...")
  analysis_output <- main_trajectory_analysis(force_rerun = FALSE)
  ...
  message("脚本在交互模式下运行")
  message("请运行: analysis_output <- main_trajectory_analysis()")
}

message("\n轨迹分析脚本v17加载完成!")
message("修复CDS数据访问:")
message("1. 添加get_cds_counts()和get_cds_logcounts()函数")
message("2. 修复tradeSeq中的数据访问问题")
message("3. 修复基因分析中的数据访问问题")
message("4. 添加SingleCellExperiment库导入")
message("5. 进一步减少基因数量（100基因用于tradeSeq，80基因用于基因分析）")